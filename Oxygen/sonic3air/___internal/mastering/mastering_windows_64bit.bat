...
%msbuildPath% ..\oxygenengine\build\_vstudio\oxygenengine.sln /property:Configuration=Release /property:Platform=x64 -verbosity:minimal
...
copy "bin\Release_x64\*.exe" "%outputDir%"
copy "source\external\discord_game_sdk\lib\x86_64\discord_game_sdk.dll" "%outputDir%"


:: Add Oxygen engine

robocopy "..\oxygenengine\_master_image_template" "%outputDir%\bonus\oxygenengine" /e
copy "..\oxygenengine\bin\Release_x64\OxygenApp.exe" "%outputDir%\bonus\oxygenengine\"
...
copy "bin\Release_x64\*.exe" "%destDir%\artifacts\bin"
copy "bin\Release_x64\*.pdb" "%destDir%\artifacts\bin"
