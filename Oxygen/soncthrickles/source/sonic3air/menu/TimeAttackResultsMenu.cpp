...
void TimeAttackResultsMenu::render()
{
	...
	if (mTime > FADE1_TIME)
	{
		...
		drawer.printText(global::mFont18, Recti(76 + std::min(roundToInt(mTime * 500) - 160, 0), 97, 0, 0), text, 1);
		...
		if (!mBetterTimes.empty())
		{
			...
			drawer.printText(global::mFont10, Recti(px, py, 0, 0), text, 1);
		}
		else if (!mWorseTimes.empty())
		{
			...
			const int py = 64;
			drawer.printText(global::mFont10, Recti(px, py, 0, 0), text, 1, roundToInt(mTime * 8.0f) % 2 ? Color::YELLOW : Color::WHITE);
		}
		...
		for (size_t line = 0; line < mMenuEntries.size(); ++line)
		{
			...
			drawer.printText(global::mFont18, Recti(46 + (int)line * 2, 150 + (int)line * 28, 0, 20), entry.mText, 4, color);
		}
	}
